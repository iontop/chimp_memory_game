...
def shuffle_grid(number_count):
    ...
    # [0,0,0,0,0,0,0,0,0] X 5rows
    grid=[[0 for col in range(columns)] for row in range(rows)] # 5 X 9
    
    number=1 #시작숫자를 1부터 number_count까지 랜덤으로 배치
    while number <= number_count:
        row_idx=randrange(0, rows) # 0이상 5미만에서 랜덤으로 뽑기
        col_idx=randrange(0, columns) # 0이상 8미만에서 랜덤으로 뽑기
...
def display_game_screen():
    ...
    if not hidden:
        elapsed_time=(pygame.time.get_ticks()-start_ticks)/1000 # 현재시간-시작버튼 클릭했을 때의 시간 = 경과시간, ms단위로 sec 단위 변경위해 1000으로 나눠줌
...
def check_buttons(pos):
    ...
    if start: # 이미 게임을 시작했으면
        check_number_buttons(pos)
    elif start_button.collidepoint(pos): #사용자가 클릭한 위치가 start_button 내에 위치하면
        start=True
...
game_font=pygame.font.Font(None, 120) # 폰트정의, None는 Default값을 의미하고 120은 글자크기
...
start_ticks=None # 시간계산 (현재 시간정보를 저장)

# 게임시작여부 판단
start=False

# 숫자 숨김 여부 (사용자가 1을 클릭했거나, 시작버튼 누른 후 일정시간이 지난 후)
hidden=False

# 게임 시작 전에 게임설정 함수 실행
setup(1) # Level 1 게임 실행 
